# 008:从判断 0.1 + 0.2 与 0.3 是否相等引发出的知识
...
```
但却输出false的时候，我们就知道又要开始学习了。

相信我们大家知道这是浮点数精度所导致的，但是其背后的原理还是要了解一下。

先上最终结果：

**两次存储时的精度丢失  +  一次运算时的精度丢失，最终导致了 0.1 + 0.2 !== 0.3**

> 本文主要参考自冴羽的[JavaScript 深入之浮点数精度](https://github.com/mqyqingfeng/Blog/issues/155#)，梳理一下，供自己学习

##  Number类型

ECMAScript 中的 Number 类型使用 IEEE754 标准（IEEE 二进制浮点数算术标准）来表示整数和浮点数值。

在 IEEE754 中，有四种表示浮点数值的方式：
